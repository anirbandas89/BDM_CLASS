...
int background_functions(
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    /* (3 p_fld) is the "relativistic" contribution to rho_fld */
    rho_r += 3.* w_fld * pvecback[pba->index_bg_rho_fld];

    /* (rho_fld - 3 p_fld) is the "non-relativistic" contribution
       to rho_fld */
    rho_m += pvecback[pba->index_bg_rho_fld] - 3.* w_fld * pvecback[pba->index_bg_rho_fld];
  }
...
int background_w_fld(
  ...
  /** - first, define the function w(a) */                          //modified by AD

  *w_fld = (1. - tanh((a - pba->a_fld)/(pba->a_fld/pba->sharpness_fld)) )/6.;
  ...
  *dw_over_da_fld = -1./ ( 6. * pba->a_fld/pba->sharpness_fld * pow(cosh((a - pba->a_fld)/(pba->a_fld/pba->sharpness_fld)), 2.) );
  ...
  *integral_fld = 0.;      //3.*((1.+pba->w0_fld+pba->wa_fld)*log(pba->a_today/a) + pba->wa_fld*(a/pba->a_today-1.));
...
int background_indices(
  ...
  pba->has_cdm = _TRUE_;   //modified by AD
...
int background_solve(
  ...
  if (pba->background_verbose > 0) {
    ...
    printf("     -> Omega0_cdm = %f\n",pba->Omega0_cdm);       //modified by AD
    printf("     -> Omega0_b = %f\n",pba->Omega0_b);
    printf("     -> Omega0_fld = %f\n",pba->Omega0_fld);
    printf("     -> Omega0_lambda = %f\n",pba->Omega0_lambda);

    printf("   ---Fluid parameters---\n");
    printf("     -> Transition redshift = %.2e\n",pba->redshift_fld);
    printf("     -> Transition sharpness = %.5f\n",pba->sharpness_fld);
    printf("     -> Fluid fraction = %.4f\n",pba->fraction_fld);
    }
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_){
    ...
    pvecback_integration[pba->index_bi_rho_fld] = rho_fld_today * pow( (pba->a_fld/a) , (3.*w_fld) ) * pow(a,-3);     //modified by AD
