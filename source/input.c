...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /** - Omega_0_cdm (CDM) */  //modified by AD:First read fluid_fraction and Omeag0_cdm. Then Omega0_cdm is multiplied by (1 - fluid_fraction). Omega0_fld is the rest, i.e., Omega0_cdm*fluid_fraction.
  class_read_double("Fluid_fraction",pba->fraction_fld);
  ...
  if (flag1 == _TRUE_)
    {
     pba->Omega0_cdm = param1 * (1. - pba->fraction_fld + 1.e-4); //AD: A small amount of CDM is always kept so that the synchronous gauge definition is not screwed up.
     pba->Omega0_fld = param1 * pba->fraction_fld;
     }
  if (flag2 == _TRUE_)
    {
     pba->Omega0_cdm = (param2/pba->h/pba->h) * (1. - pba->fraction_fld + 1.e-4);
     pba->Omega0_fld = (param2/pba->h/pba->h) * pba->fraction_fld;
     }

  Omega_tot += pba->Omega0_cdm + pba->Omega0_fld;
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  /////////////////////////////////////////////////////////////   modified by AD
  //class_read_double("Fluid_fraction",pba->fraction_fld);
  /** - Once Omega0_fld fraction is read, determine Omega0_cdm from Planck value 0.12038. Omega0_c=0.12038 - Omega0_fld. 
   *    A small amount of CDM is always kept so that the synchronous gauge definition is not screwed up. -AD */
/*  class_call(parser_read_double(pfc,"Omega_fld",&param2,&flag2,errmsg),
             errmsg,
             errmsg);

  pba->Omega0_fld = param2 * pba->fraction_fld;
  Omega_tot += pba->Omega0_fld;

  pba->Omega0_cdm = (param2 - pba->Omega0_fld) + 0.000001;
...
*/
  ...
  */
  ...
  /*if (flag2 == _TRUE_){
    pba->Omega0_fld = param2 * pba->fraction_fld;           //modified by AD
    Omega_tot += pba->Omega0_fld;
  }*/
  ...
  /*else if (flag2 == _FALSE_) {
   ...
   //printf("Omega0_g-> %.5e, Omega0_b-> %.5e, Omega0_lambda-> %.5e, Omega0_scf-> %.5e\n", pba->Omega0_g, pba->Omega0_b, pba->Omega0_lambda, pba->Omega0_scf);
   //printf("Omega0_fld-> %.5e, Omega0_cdm-> %.5e, Omega_K-> %.5e, Omega_tot-> %.5e\n", pba->Omega0_fld, pba->Omega0_cdm, pba->Omega0_k, Omega_tot);
  /*
  ...
  */
  ...
  printf("-----------------------------------------------------------------\n");  //modified by AD. Takes either transition redshift or scale factor as input. Complains if both are given.
    //class_read_double("Transition_redshift",pba->redshift_fld);

  /////////////////////////////////////////////////////////////////////////////////////////////  
  class_call(parser_read_double(pfc,"Transition_redshift",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Transition_scale_factor",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Transition_redshift or Transition_scale_factor, choose one!");
  if (flag1 == _TRUE_){
      pba->redshift_fld = param1;
      pba->a_fld = 1./(1. + pba->redshift_fld);
      }
  else {
      class_test((param2 == 0.),
             errmsg,
             "You have entered Transition_scale_factor = 0 which makes the Transition_redshift infinity! Instead, enter a sufficiently small number.");
      pba->a_fld = param2;
      pba->redshift_fld = -1. + 1./pba->a_fld;
      }

  class_read_double("Transition_sharpness",pba->sharpness_fld);
  /////////////////////////////////////////////////////////////////////////////////////////////


    //class_read_double("cs2_fld",pba->cs2_fld);
  /*if (pba->fraction_fld != 0.) {                         //modified here by AD, but not needed in BDM cosmology.
...
int input_default_params(
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_fld = 0.;          //modified by AD
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr-pba->Omega0_fld;

  pba->fraction_fld = 0.;
  pba->a_today = 1.;
  pba->redshift_fld=100000.;
  pba->a_fld = 1./(1. + 100000.);
  pba->sharpness_fld=100.;
  //pba->cs2_fld=1.;
  pba->use_ppf = _FALSE_;
...
int input_default_precision ( struct precision * ppr ) {
  ...
  ppr->a_ini_over_a_today_default = 1.e-16;
