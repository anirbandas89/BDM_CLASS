...
int perturb_init(
  ...
  /////////////////////////////////////////////////////////////////////////
  // modified by AD: this checks are not required for our model
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double fracnu,fracg,fracb,fraccdm,fracfld,om;       //modified by AD. fracfld added.
  ...
  if (_scalars_) {
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      rho_r += ppw->pvecback[pba->index_bg_rho_fld];
    }
    ...
    /* f_fld = Omega_fld(t_i) / Omega_r(t_i) */
    fracfld = ppw->pvecback[pba->index_bg_rho_fld]/rho_r;    //modified by AD
    //printf("fracg->%f, fracnu->%f, fracfld->%f\n", fracg, fracnu, fracfld);
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* fluid (assumes wa=0, if this is not the case the
         fluid will catch anyway the attractor solution) */     //modified by AD, only in adiabatic. Fluid initial conditions
      if (pba->has_fld == _TRUE_) {

        class_call(background_w_fld(pba,a,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);

        if (pba->use_ppf == _FALSE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_fld] = ppw->pv->y[ppw->pv->index_pt_delta_g];

          ppw->pv->y[ppw->pv->index_pt_theta_fld] = ppw->pv->y[ppw->pv->index_pt_theta_g];

        //if ( abs(k-0.7) < 0.0000001 && (mycounter1)==0 ){ printf("From initial routine, tau->%f, delta_g-> %f, delta_fld-> %f\n", tau, ppw->pv->y[ppw->pv->index_pt_delta_g],ppw->pv->y[ppw->pv->index_pt_delta_fld]);
          //                             mycounter1 +=1; }
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      //modified by AD. Fluids added to the radiation parts of delta_tot and velocity_tot.
      delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur + fracfld*ppw->pv->y[ppw->pv->index_pt_delta_fld] +rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fraccdm*delta_cdm))/(1.+rho_m_over_rho_r);

      velocity_tot = ((4./3.)*(fracg*ppw->pv->y[ppw->pv->index_pt_theta_g]+fracnu*theta_ur + fracfld*ppw->pv->y[ppw->pv->index_pt_theta_fld]) + rho_m_over_rho_r*fracb*ppw->pv->y[ppw->pv->index_pt_theta_b])/(1.+rho_m_over_rho_r);
      ...
      if ((pba->has_fld == _TRUE_) && (pba->use_ppf == _FALSE_)) {
        ...
        ppw->pv->y[ppw->pv->index_pt_delta_fld] -= 3*(1.+w_fld)*a_prime_over_a*alpha;   //modified by AD: a sign change here since for w_fld=1/3 this should be identical to photon
  ...
  if (_scalars_) {
    ...
    */
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      ppw->delta_p += w_fld * ppw->delta_rho_fld;          //modified by AD
    ...
    if (ppt->has_source_delta_m == _TRUE_) {
      ...
      /* include fluid */                   //modified by AD. The factors of (1-3*w) are multiplied so that they contribute only when nonrelativistic

      if (pba->has_fld == _TRUE_) {
        class_call(background_w_fld(pba,a,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
        delta_rho_m += ppw->pvecback[pba->index_bg_rho_fld]*y[ppw->pv->index_pt_delta_fld] * (1.-3.*w_fld);
        rho_m += ppw->pvecback[pba->index_bg_rho_fld] * (1.-3.*w_fld);
      }
    ...
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
      ...
      if (pba->has_fld == _TRUE_) {
        class_call(background_w_fld(pba,a,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
        rho_plus_p_theta_m += (1.+w_fld) * ppw->pvecback[pba->index_bg_rho_fld]*y[ppw->pv->index_pt_theta_fld] * (1.-3.*w_fld);
        rho_plus_p_m += (1.+w_fld) * ppw->pvecback[pba->index_bg_rho_fld] * (1.-3.*w_fld);
      }
...
int perturb_print_variables(double tau,
  ...
  double delta_g,theta_g,shear_g,l4_g,pol0_g,pol1_g,pol2_g,pol4_g;        //modified by AD
  ...
  if (_scalars_) {
    ...
    if (pba->has_fld == _TRUE_){                 //modified by AD
      class_call(background_w_fld(pba,pvecback[pba->index_bg_a],&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
      delta_fld = ppw->delta_rho_fld/pvecback[pba->index_bg_rho_fld];                           //y[ppw->pv->index_pt_delta_fld];
      theta_fld = ppw->rho_plus_p_theta_fld/(1. + w_fld)/pvecback[pba->index_bg_rho_fld];       //y[ppw->pv->index_pt_theta_fld];
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_fld == _TRUE_) {                                         //modified by AD
        ...
        delta_fld -= 3. * (1. + w_fld) * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
        theta_fld += k*k*alpha;
      }
  ...
  if (_scalars_) {
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        cs2 = w_fld;      //modified by AD, not using the CLASS sound speeds & equations. See Ma & Bertschinger.
        /** - ----> fluid density */
        ...
        dy[pv->index_pt_delta_fld] =
          -(1.+w_fld)*(y[pv->index_pt_theta_fld]
            +metric_continuity
             );
        //if ( abs(k-0.7) < 0.0000001 && (mycounter)==0 ){ printf("From derivs routine, tau->%f, delta_g-> %f, delta_fld-> %f\n", tau, y[pv->index_pt_delta_g],y[pv->index_pt_delta_fld]);
        ...
        /*dy[pv->index_pt_theta_fld] =
          ...
          +metric_euler;*/
        dy[pv->index_pt_theta_fld] =
          -(1.-3.*w_fld)*a_prime_over_a*y[pv->index_pt_theta_fld]
            -w_prime_fld/(1.+w_fld)*y[pv->index_pt_theta_fld]
            +w_fld*k2/(1.+w_fld)*y[pv->index_pt_delta_fld]
            +metric_euler
            ;
      }
      else {
